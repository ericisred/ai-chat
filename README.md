...
本项目采用**策略模式（Strategy Pattern）与工厂模式（Factory Pattern）**设计，实现业务层与底层 LLM SDK 的解耦；引入全新的 **`memory/` 双层架构**，支持长对话滚动摘要压缩与历史会话断点恢复。
...
- 💾 **SQLite 对话持久化**：基于 SQLite3 支持 `sessions` 与 `messages` 完整数据落盘、惰性会话创建 (Lazy Session) 以及跨会话续聊。
